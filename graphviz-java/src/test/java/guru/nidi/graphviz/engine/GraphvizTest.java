...
class GraphvizTest {
    ...
    void executeWithTotalMemory() {
        final Graph graph = graph().with(node("a").link("b"));
        final String result = Graphviz.fromGraph(graph).totalMemory(32000).render(Format.SVG).execute().string;
    ...
    void executeWithoutTotalMemory() {
        final Graph graph = graph().with(node("a").link("b"));
        final String result = Graphviz.fromGraph(graph).render(Format.SVG).execute().string;
