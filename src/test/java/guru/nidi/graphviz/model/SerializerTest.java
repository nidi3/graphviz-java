...
public class SerializerTest {
    ...
    public void context() {
        CreationContext.begin()
                ...
                .links().attr("l", "z");
        assertGraph("graph 'x' {\n'g'='x'\n'x' ['n'='y','bla'='blu']\n'y' ['n'='y']\n'x' -- 'y' ['l'='z']\n}", graph("x")
                .withNodes(node("x").attr("bla", "blu").link(node("y"))));
    }

    @Test
    public void subgraph() {
        assertGraph("graph 'x' {\nsubgraph 'x' {\n'x' ['bla'='blu']\n}\n}", graph("x")
                .withGraphs(graph("x").withNodes(node("x").attr("bla", "blu"))));
    }

    @Test
    public void namelessSubgraph() {
        assertGraph("graph 'x' {\n{\n'x' ['bla'='blu']\n}\n}", graph("x")
                .withGraphs(graph().withNodes(node("x").attr("bla", "blu"))));
    }

    @Test
    public void simpleEdge() {
        assertGraph("graph 'x' {\n'x' -- 'y'\n}", graph("x")
                .withNodes(node("x").link(node("y"))));
    }

    @Test
    public void attrEdge() {
        assertGraph("graph 'x' {\n'x' -- 'y' ['bla'='blu']\n}", graph("x")
                .withNodes(node("x").link(to(node("y")).attr("bla", "blu"))));
    }

    @Test
    public void graphEdgeStart() {
        assertGraph("graph 'x' {\nsubgraph 'y' {\n'z' -- 'a'\n} -- 'x':n\n}", graph("x").withGraphs(
                graph("y").withNodes(node("z").link(
    ...
    public void graphEdgeEnd() {
        assertGraph("graph 'x' {\n'x':n -- subgraph 'y' {\n'z' -- 'a'\n}\n}", graph("x").withNodes(
                node("x").link(between(loc(NORTH),
                        graph("y").withNodes(node("z").link(node("a")))))));
    }

    @Test
    public void graphEdge() {
        assertGraph("graph 'x' {\nsubgraph 'y' {\n'z' -- 'a'\n} -- subgraph 'y2' {\n'z2' -- 'a2'\n}\n}", graph("x").withGraphs(
                graph("y").withNodes(node("z").link(node("a"))).link(
                        graph("y2").withNodes(node("z2").link(node("a2"))))));
    }

    @Test
    public void compassEdge() {
        assertGraph("graph 'x' {\n'x':sw -- 'y':ne\n}", graph("x")
                .withNodes(node("x").link(between(loc(SOUTH_WEST), node("y").loc(NORTH_EAST)))));
    }

    @Test
    public void recordEdge() {
        assertGraph("graph 'x' {\n'x':'r1' -- 'y':'r2'\n}", graph("x")
                .withNodes(node("x").link(between(loc("r1"), node("y").loc("r2")))));
    }

    @Test
    public void compassRecordEdge() {
        assertGraph("graph 'x' {\n'x':'r1':sw -- 'y':'r2':ne\n}", graph("x")
                .withNodes(node("x").link(
    ...
    public void complexEdge() {
        assertGraph("digraph 'x' {\n'x' -> 'y'\n'y' -> 'z'\n'a' -> 'x'\n}", graph("x").directed()
                .withNodes(node("x").link(node("y").link(node("z"))))
                .withNodes(node("a").link(node("x"))));
    }
